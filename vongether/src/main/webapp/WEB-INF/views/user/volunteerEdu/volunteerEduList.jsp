		...
		function getVolunteerList() {
			$.ajax({
				...
				success : function(data) {
					...
					//console.log(json1[0]);
					var json2 = json1[0].items[0].item[0];
					console.log(json2);
					//crclmNm
					//crclmRegistNo
					//edcBgnde
		...
		function makeVolunteerList(data) {
			...
			if (data != null) {
				...
				for (var i = 1; i < data.volunteerList.length; i++) {
					...
					htmlStr += '			<h4><b><a id="'
							+ data.volunteerList[i].progrmRegistNo
							+ '" href="./detail.do?num='
							+ data.volunteerList[i].progrmRegistNo + '">'
							+ data.volunteerList[i].progrmSj + '</a></b></h4>';
					htmlStr += '			<p>' + data.volunteerList[i].nanmmbyNm
							+ '</p>';
					htmlStr += '			<p>' + data.volunteerList[i].progrmBgnde
							+ ' ~  ' + data.volunteerList[i].progrmEndde
							+ '</p>';
					...
					htmlStr += '			<p class="'+ color +'">' + stateString
							+ '</p>';
					htmlStr += '			<p><a href="./detail.do?num='
							+ data.volunteerList[i].progrmRegistNo
							+ '" class="btn btn-primary btn-outline with-arrow" program-code="'
							+ data.volunteerList[i].progrmRegistNo
							+ '">Read More<i class="icon-arrow-right"></i></a></p>';
					htmlStr += '		</div>';
					htmlStr += '	</div>';
					htmlStr += '</div>';
			...
			for (var nowPage = data.pagination.startPage; nowPage <= data.pagination.endPage; nowPage++) {
				if (nowPage == data.pagination.curPage) {
					htmlStr2 += '<button type="button" class="btn btn-primary active mvpage" move-page-no="'+ nowPage +'">'
							+ nowPage + '</button>';
				} else if (nowPage != data.pagination.curPage) {
					htmlStr2 += '<button type="button" class="btn mvpage" move-page-no="'+ nowPage +'">'
							+ nowPage + '</button>';
				}
			}

			if (data.pagination.curRange != data.pagination.totalPage
					&& data.pagination.totalPage > 0) {
